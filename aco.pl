...
/* arcPref und arcTau werden als globale Variablen mittels assert angelegt (Teil 3).
 arcTau beschreibt die Intensität der Trails, diese werden nach dem Lauf einer 
...
stableSolCountForLoopStop(20).
/******************************************************************************
Teil 1: Boilerplate*/

/* Initialisierung mit Anschließenden Start.*/ 
main :-
    allArcs(XYs), initTP(XYs),
...
initTP([]).
initTP([X:Y|XYs]) :- assert(arcTau(X, Y, 1)), updateP(X, Y), initTP(XYs).
...
/* Die Kosten werden entnormiert und der Lösungspfad wird ausgegeben, um mittels
Gnuplot anzeigt zu werden. Anschließend werden die Pheromone ausgegeben. */
writeSol :-
    bestSol(Path, _),
